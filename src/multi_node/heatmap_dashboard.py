...
def on_message(client, userdata, msg):
    ...
    try:
        ...
        print(f"Raw MQTT message - Topic: {msg.topic}, Payload: {msg.payload.decode('utf-8')}")
        ...
        if all(v is not None for v in [temp, hum, pos_x, pos_y]):
            ...
            cursor.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?)
            ''', (node_id, dt_object.strftime("%Y-%m-%d %H:%M:%S"), temp, hum, pos_x, pos_y))
            ...
            for j in range(len(x_grid)):
                ...
                for node_id, data in node_data.items():
                    ...
                    if distance <= NODE_RADIUS:
                        ...
                        weight = 1.0 / (distance + 0.1)  # +0.1 untuk avoid division by zero
        ...
        html.Div(
            ...
            children=[
                ...
                html.P(
                    ...
                    style={"textAlign": "center", "fontSize": "1.1em", "color": "#cccccc"}
                ),
            ],
        ),
...
def update_dashboard(_):
    ...
    with data_lock:
        ...
        for node_id, data in node_data.items():
            ...
            nodes_status.append(
                html.Div([
                    html.Span(f"● {node_id}", style={"color": status_color, "fontWeight": "bold", "marginRight": "15px"}),
                    html.Span(f"T: {last_temp}°C", style={"marginRight": "10px"}),
                    html.Span(f"H: {last_hum}%", style={"marginRight": "10px"}),
                    html.Span(f"Pos: ({pos_x}, {pos_y})", style={"marginRight": "10px"}),
                    html.Span(f"Status: {data.get('status', 'unknown')}", style={"color": "#cccccc"})
                ], style={"marginBottom": "8px", "display": "inline-block", "width": "100%"})
            )
    
    if not nodes_status:
        nodes_status = [html.Div("Menunggu data dari nodes...", style={"color": "#888888"})]
    ...
    for node_id, data in nodes.items():
        if 'pos_x' in data and 'pos_y' in data:
            ...
            node_text.append(f"{node_id}<br>T: {data['temperature']}°C<br>H: {data['humidity']}%")
            node_colors.append('#00ff00' if data.get('status') == 'online' else '#ff4444')
